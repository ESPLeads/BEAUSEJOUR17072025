...
import { SalesBulkEditModal } from './SalesBulkEditModal';
...
export function SalesModule({ 
  ...
  const handleSaveEdit = async (updates: Partial<RegisterSale>) => {
    ...
    console.log(`🔄 Saving edit for sale: ${editingSale.id}`);
    console.log(`📝 Updates:`, updates);
    
    setIsUpdating(true);
    try {
      // Validate that the sale still exists in current data
      const saleStillExists = registerSales.some(sale => sale.id === editingSale.id);
      if (!saleStillExists) {
        console.warn(`⚠️ Sale ${editingSale.id} no longer exists in current registerSales data`);
        showToast('error', 'Cette vente n\'existe plus dans les données locales. Actualisation en cours...');
      ...
      if (success === false) {
        showToast('error', 'Cette vente n\'existe plus dans la base de données. Elle a peut-être été supprimée.');
        setShowEditModal(false);
        setEditingSale(null);
        onRefreshData();
      } else {
      ...
      showToast('error', `Erreur lors de la modification: ${error.message || 'Erreur inconnue'}`);
    } finally {
      setIsUpdating(false);
    }
  };
  ...
  const handleSaveCategorization = async (category: string, subcategory?: string) => {
    ...
    try {
      await onCategorizeSales(Array.from(selectedSales), category, subcategory);
  ...
  const handleSaveBulkEdit = async (updates: Partial<RegisterSale>) => {
    if (selectedSales.size === 0 || !onUpdateSale) return;

    setIsBulkEditing(true);
    try {
      console.log(`🔄 Starting bulk edit for ${selectedSales.size} sales`);
      console.log(`📝 Updates to apply:`, updates);
      console.log(`📋 Selected sale IDs:`, Array.from(selectedSales));
      
      // Validate all selected sale IDs exist in current data
      const validSaleIds = Array.from(selectedSales).filter(saleId => {
        const saleExists = registerSales.some(sale => sale.id === saleId);
        if (!saleExists) {
          console.warn(`⚠️ Sale ID ${saleId} not found in current registerSales data`);
        }
        return saleExists;
      });
      
      console.log(`✅ Valid sale IDs: ${validSaleIds.length}/${selectedSales.size}`);
      
      if (validSaleIds.length === 0) {
        showToast('error', 'Aucune vente valide sélectionnée. Les ventes ont peut-être été supprimées.');
        setShowBulkEditModal(false);
        setSelectedSales(new Set());
        return;
      }
      
      // Apply updates to each selected sale
      const updateResults = await Promise.all(
        validSaleIds.map(async (saleId) => {
          try {
            console.log(`🔄 Updating sale: ${saleId}`);
            const result = await onUpdateSale(saleId, updates);
            console.log(`✅ Update result for ${saleId}:`, result);
            return result;
          } catch (error) {
      ...
      const successCount = updateResults.filter(result => result !== false).length;
      const notFoundCount = updateResults.filter(result => result === false).length;
      
      console.log(`📈 Success: ${successCount}, Not found: ${notFoundCount}`);
      ...
      if (successCount > 0 && notFoundCount === 0) {
        showToast('success', `${successCount} vente(s) modifiée(s) avec succès`);
      } else if (successCount > 0 && notFoundCount > 0) {
        showToast('warning', `${successCount} vente(s) modifiée(s), ${notFoundCount} vente(s) introuvable(s)`);
      } else {
        showToast('error', `Aucune vente n'a pu être modifiée. ${notFoundCount} vente(s) introuvable(s) dans la base de données.`);
      }
      
      // Refresh data to show updated sales
      onRefreshData();
    } catch (error) {
      console.error('Error bulk editing sales:', error);
      showToast('error', `Erreur lors de la modification en lot: ${error.message || 'Erreur inconnue'}`);
    } finally {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      const selectedIds = Array.from(selectedSales).map(id => String(id));
      console.log(`🗑️ Attempting to archive ${selectedIds.length} sales with IDs:`, selectedIds);
      
      // Call the delete function
      const result = await onDeleteSales(selectedIds);
      
      // Check the result
      if (!result.success) {
        if (result.errorCount > 0) {
          throw new Error(`L'archivage a échoué pour ${result.errorCount} vente(s). Vérifiez la console pour plus de détails.`);
        } else {
          throw new Error('Aucune vente n\'a pu être archivée. Vérifiez les permissions Firebase et la structure des collections.');
        }
      }
      
      console.log(`✅ Archiving completed: ${result.successCount} successful, ${result.notFoundCount} not found, ${result.errorCount} errors`);
      ...
      if (result.successCount > 0) {
        message += `${result.successCount} vente(s) archivée(s) avec succès`;
      }
      if (result.notFoundCount > 0) {
        if (message) message += '. ';
        message += `${result.notFoundCount} vente(s) déjà supprimée(s) ou introuvable(s)`;
      }
      if (result.errorCount > 0) {
        if (message) message += '. ';
      ...
      showToast(result.errorCount > 0 ? 'warning' : 'success', message);
      
      // Force refresh data after a short delay to ensure Firestore has propagated changes
      await onRefreshData();
      
    } catch (error) {
      console.error('❌ Error archiving sales:', error);
      
      showToast('error', error.message || 'Une erreur est survenue lors de l\'archivage');
  ...
  return (
    <div className="space-y-6">
      ...
      {showBulkEditModal && onUpdateSale && (
        <SalesBulkEditModal
          selectedSales={Array.from(selectedSales).map(id => 
            registerSales.find(sale => sale.id === id)!
          ).filter(Boolean)}
          selectedCount={selectedSales.size}
          ...
          >
            ...
            >
              <div className="flex items-center space-x-3 mb-4">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-white">Confirmer l'archivage</h3>
                  <p className="text-gray-400 text-sm">Les ventes seront archivées et non supprimées définitivement</p>
                </div>
              </div>

              <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-4 mb-6">
                <h4 className="text-red-400 font-semibold mb-2">Données à archiver :</h4>
                <div className="text-gray-300 text-sm space-y-1">
                  <div>• <strong>{selectedSales.size}</strong> vente(s) sélectionnée(s) pour archivage</div>
                  ...
                  <div>• Les ventes seront archivées pour audit mais retirées de l'interface</div>
                  <div>• Les quantités vendues seront recalculées automatiquement</div>
                  <div>• Les données archivées restent accessibles pour les rapports d'audit</div>
                  <div className="mt-2 text-yellow-400">• Les ventes archivées ne seront plus visibles dans cette interface</div>
                </div>
              </div>
              
              <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-4 mb-6">
                <h4 className="text-blue-400 font-medium mb-2">Information technique :</h4>
                <div className="text-gray-300 text-sm space-y-1">
                  <div>• Les ventes seront déplacées vers la collection <strong>archived_sales</strong> dans Firestore</div>
                  <div>• Chaque vente archivée conserve toutes ses données originales</div>
                  <div>• Des métadonnées d'archivage sont ajoutées (date, raison, etc.)</div>
                  <div>• Si l'opération échoue, cliquez sur <strong>"Initialiser Firebase"</strong> dans le Tableau de Bord</div>
                  <div>• Vérifiez que la collection "archived_sales" existe dans votre console Firebase</div>
                </div>
              </div>
