...
export function ImportModule({ onImportSales, onRefreshData, existingSales = [] }: ImportModuleProps) {
  ...
  const [lastImport, setLastImport] = useState<{
    ...
    try {
      const saved = localStorage.getItem('globalva_last_import');
      if (saved) {
    ...
    <div className="space-y-6">
      ...
      {lastImport && (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className="bg-gradient-to-r from-orange-500/20 to-red-500/20 backdrop-blur-xl 
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h3 className="text-lg font-semibold text-white">Annuler le Dernier Import</h3>
                <p className="text-gray-400 text-sm">
                  Dernier import: {lastImport.count} vente(s) le {format(lastImport.timestamp, 'dd/MM/yyyy à HH:mm')}
                </p>
              </div>
            </div>
            ...
            <button
              ...
              className="bg-gradient-to-r from-orange-500 to-red-500 text-white font-semibold 
                         py-3 px-6 rounded-xl hover:from-orange-600 hover:to-red-600 
          ...
          <div className="mt-4 bg-orange-500/10 border border-orange-500/20 rounded-xl p-4">
            <div className="flex items-start space-x-3">
              ...
              <div>
                <h5 className="text-orange-400 font-semibold">⚠️ Action Irréversible</h5>
                <p className="text-gray-300 text-sm mt-1">
                  Cette action supprimera définitivement toutes les ventes du dernier import.
                  Les données seront archivées mais ne pourront pas être restaurées automatiquement.
                  Utilisez cette fonction uniquement en cas d'erreur d'import (mauvais fichier, doublons, etc.).
                </p>
              </div>
            </div>
          </div>
